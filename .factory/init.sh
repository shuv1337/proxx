...
# Source user's global env for FACTORY_API_KEY
if [ -f "$HOME/.env" ]; then
  set -a
  source "$HOME/.env"
  set +a
...
# Ensure FACTORY_API_KEY is in .env if available in environment
if [ -n "${FACTORY_API_KEY:-}" ] && ! grep -q "^FACTORY_API_KEY=" .env 2>/dev/null; then
  echo "FACTORY_API_KEY=${FACTORY_API_KEY}" >> .env
fi
